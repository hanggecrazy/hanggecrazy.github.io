...
class Bot extends \Baidu\Duer\Botsdk\Bot {
    ...
    public function __construct($postData = []) {
        ...
        $this->addSessionEndedHandler(function(){
            $this->log->setField('return null:', $this->request->getQuery());
            return null; 
        });

        // 在匹配到intent的情况下，首先询问月薪
        //$this->addIntentHandler('personal_income_tax.inquiry', function() {
        $this->addIntentHandler('tax.inquiry', function() {
            if(!$this->getSlot('sys.number')) {
                ...
                // 在存在sys.number槽位的情况下，首先验证sys.number槽位值是否合法，然后询问sys.city槽位
                $ret = $this->checkMonthlysalary();
                if ($ret != null) {
                    $this->log->setField('salary not null:', $ret);
                    return $ret;
                }
                ...
                // 在存在sys.city槽位的情况下，首先验证sys.city槽位是否在支持的城市列表中，然后询问compute_type槽位
                $ret = $this->checkLocation();
                if ($ret != null) {
                    $this->log->setField('city not null:', $ret);
                    return $ret;
                }
    ...
    public function checkMonthlysalary() {
        $sysnumber = $this->getSlot('sys.number');
        $this->log->setField('number', $sysnumber);
        $value = intval($sysnumber);
        if ($value <= 0) {
            $this->nlu->ask('sys.number');
            return [
    ...
    public function checkLocation() {
        // 判断是否在支持的城市列表中
        $syscity = json_decode($this->getSlot('sys.city'), true);
        $this->log->setField('city', $this->getSlot('sys.city'));

        if (!in_array($syscity['city'], self::$city)) {
            $this->nlu->ask('sys.city');
            return [
                //'card' => new TextCard("该城市不存在，请重新选择城市：")
                'card' => new TextCard($syscity['city'])
            ];
        }
    }
    ...
    public function compute() {
        ...
        $ret = $this->checkMonthlysalary();
        if ($ret != null) {
            $this->log->setField('money not null:', $ret);
            return $ret;
        }
        // sys.city槽位存在的情况下，判断该城市是否存在
        $ret = $this->checkLocation();
        if ($ret != null) {
            $this->log->setField('location not null:', $ret);
            return $ret;
        }
        // compute_type槽位存在的情况下，判断计算类型是否存在
        $compute_type = $this->getSlot('compute_type');
        $this->log->setField('compute_type', $compute_type);
        ...
        $sysnumber = intval($this->getSlot('sys.number'));
        $syscity = json_decode($this->getSlot('sys.city'), true);
        // 构造请求的url
        $url = self::$url 
            . '&monthlysalary=' . $sysnumber 
            . '&location=' . $syscity['city'] 
